...
export function ModalRegistrar({ tipoInicial, onClose, movimientoEditar, onGuardado }: Props) {
    ...
    return createPortal(
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0, 0, 0, 0.4)', backdropFilter: 'blur(8px)', padding: '16px' }} onClick={e => e.target === e.currentTarget && onClose()}>
            <div style={{ background: 'var(--white)', borderRadius: '32px', width: '100%', maxWidth: '460px', maxHeight: '90vh', display: 'flex', flexDirection: 'column', boxShadow: 'var(--shadow-lg)', overflow: 'hidden' }}>
                ...
                <div style={{ padding: '32px', overflowY: 'auto' }}>
                    ...
                    <div style={{ marginBottom: '32px' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
                            ...
                            {/* Botoón Añadir */}
                            {creatingCat ? (
                                <div style={{ gridColumn: 'span 3', padding: '16px', borderRadius: '20px', background: 'var(--bg)', display: 'flex', flexDirection: 'column', gap: '12px', border: '1px solid var(--border)' }}>
                    ...
                    {/* ── NOTA & FECHA FLOTANTE ── */}
                    {showExtras ? (
                        <div style={{ background: 'var(--bg)', borderRadius: '24px', padding: '24px', marginBottom: '16px' }}>
                            <div style={{ marginBottom: '16px' }}>
                                <label style={{ fontSize: '12px', fontWeight: 700, color: 'var(--t2)', display: 'block', marginBottom: '8px' }}>Fecha de Operación</label>
                                <div style={{ display: 'flex', alignItems: 'center', background: 'var(--white)', borderRadius: '12px', padding: '0 12px', border: '1px solid var(--border)' }}>
                                    <Calendar size={16} color="var(--t3)" />
                                    <input type="date" value={fecha} onChange={e=>setFecha(e.target.value)} style={{ flex: 1, border: 'none', background: 'transparent', padding: '12px', fontSize: '14px', fontWeight: 500, color: 'var(--t1)', outline: 'none' }} />
                                </div>
                            </div>
                            <div>
                                <label style={{ fontSize: '12px', fontWeight: 700, color: 'var(--t2)', display: 'block', marginBottom: '8px' }}>Observaciones</label>
                                <textarea value={nota} onChange={e=>setNota(e.target.value)} rows={2} placeholder="Escribe detalles adicionales..." style={{ width: '100%', border: '1px solid var(--border)', borderRadius: '12px', padding: '12px', fontSize: '14px', fontFamily: 'var(--font)', outline: 'none', resize: 'none' }} />
                            </div>
                        </div>
                    ) : (
                        <button onClick={() => setShowExtras(true)} style={{ width: '100%', padding: '16px', borderRadius: '20px', border: '1px dashed var(--border)', background: 'transparent', color: 'var(--t2)', fontSize: '13px', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', cursor: 'pointer', marginBottom: '16px' }}>
                            <Edit3 size={16} /> Agregar nota o cambiar fecha
                        </button>
                    )}
